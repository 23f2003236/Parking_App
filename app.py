...
with app.app_context():
    ...
    if not User.query.filter_by(username='admin').first():
        ...
        admin = User(username='admin', password= hashed.decode('utf-8'), email='admin@example.com', is_admin=True )
...
def calculate_current_cost(start_time, hourly_rate):
    ...
    parked_time = duration.total_seconds() / 3600
    billing = int(parked_time) + (1 if parked_time % 1 > 0 else 0) # Rounded up to next hour
...
def register():
    ...
    user_account = User( username=username, password=hashed, email=email, phone=phone, address=address, is_admin=False)
...
def admin_dashboard():
    ...
    return render_template('admin_dashboard.html', parking_lots=lots, lot_availability=avaialble, total_lots=len(lots), total_spots=total_spots,
                            occupied_spots=busy_spots, available_spots=free_spots )
...
def add_lot():
    ...
    if spots <= 0 or price < 0:
        ...
        return render_template('parking_lot_form.html', lot=None)
    new_lot = ParkingLot( location_name=name,address=address,pin_code=pin, price=price, number_of_spots=spots,status = 'Active' )
    db.session.add(new_lot)
    db.session.flush() #we need flush to get the id of the new lot for making the spots
    # creating spots
    for num in range(1, spots + 1):
        spot = ParkingSpot(lot_id=new_lot.id,spot_number=num,capacity=1,status='A',occupied = 0)
        db.session.add(spot)
    db.session.commit()
...
def edit_lot(lot_id):
    ...
    if new_count > current_spots:
        ...
        for num in range(add_spot):
            new_spot = ParkingSpot(lot_id=lot_id,spot_number=current_spots + num + 1, capacity=1, status='A', occupied = 0)
            db.session.add(new_spot)
            msg = f'Added {add_spot} new spots to "{lot.location_name}".'

    elif new_count < current_spots:
        busy_spots = ParkingSpot.query.filter_by(lot_id=lot_id, status='O').count()
        if busy_spots > 0:
            flash(f"can't reduce spots, {busy_spots} spots are occupied", 'danger')
            return render_template('parking_lot_form.html', lot=lot)
        # remove extra spots
        delete_spot = ParkingSpot.query.filter(ParkingSpot.lot_id == lot_id, ParkingSpot.spot_number > new_count).all()
        for spot in delete_spot:
            db.session.delete(spot)
        msg = f'Removed {current_spots - new_count} spots from "{lot.location_name}".'

    else:
        msg = f'Updated basic info for "{lot.location_name}".'
...
def admin_view_user(user_id):
    ...
    money_spent = sum(r.parking_cost for r in history if r.parking_cost) if history else 0
    avg_cost = money_spent / len(history) if history else 0
    ...
    if completed:
        ...
        for reservation in completed:
            ...
            total_park_time += session_duration.total_seconds()
        
        avg_sec = total_park_time / len(completed)
        avg_duration = f'{int(avg_sec // 3600)}h {int((avg_sec % 3600) // 60)}m'
    else:
        avg_duration = 'N/A'
    
    return render_template('admin_view_user.html', user=user, parking_history=history, total_cost=money_spent, avg_cost=avg_cost, avg_duration=avg_duration )
...
def admin_add_user():
    ...
    if user_exist:
        ...
        return render_template('admin_add_user.html')
    hashed = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
    user = User( username=username,password=hashed, email=email, phone=phone, address=address,is_admin=False)
    db.session.add(user)
    db.session.commit()
...
def admin_delete_user(user_id):
    ...
    if reservation:
        flash(f'Sorry , You Cannot delete user "{user.username}" because he has a active reservation .', 'danger')
        return redirect(url_for('admin_view_users'))
...
def view_lot_spots(lot_id):
    ...
    return render_template('parking_lot_spots.html', lot = ParkingLot.query.get_or_404(lot_id), spots=spots, available_count=free_spots, occupied_count=busy_spots)

@app.route('/admin/spots/<int:spot_id>')
@admin_required
def view_parking_spot(spot_id):
    ''' we are checking if the spot is occupied or not ? if spot are occupied, then find the reservation 
...
def admin_summary():
    ...
    for lot in ParkingLot.query.all():
        lot_id = lot.id
        location_names.append(lot.location_name)
        total = ParkingSpot.query.filter_by(lot_id=lot_id).count()
        occupied = db.session.query(ParkingSpot).join(Reservation).filter( ParkingSpot.lot_id == lot_id,Reservation.leaving_time.is_(None),
        ...
        lot_data[lot_id] = { 'name': lot.location_name,'total_spots': total, 'occupied_spots': occupied, 'available_spots': total - occupied, 'utilization': usage / 100, 
                                'revenue': lot_revenue}
    
    return render_template('admin_summary.html', total_lots=lots, total_spots=total_spots, occupied_spots=busy, available_spots=free, total_users=users,
                            total_revenue=revenue, lot_summary=lot_data, lot_names=location_names, utilization_data=usage, revenue_data=revenues )
...
def user_dashboard():
    user = User.query.get_or_404(session['user_id'])
    current_booking = Reservation.query.filter_by( user_id=user.id, leaving_time = None).first()
    old_bookings = Reservation.query.filter( Reservation.user_id == user.id,Reservation.leaving_time != None).order_by(Reservation.leaving_time.desc()).all()
    ...
    return render_template('user_dashboard.html', user=user, active_reservation=current_booking, past_reservations=old_bookings, available_lots=lots,
                            lot_availability=available, calculate_duration=calculate_duration )

@app.route('/user/book', methods=['POST'])
@login_required
def book_spot():
    '''  We are checking if the user has an active reservation or not, if yes, then we are not allowing the user to book a new reservation .if no, then we are allowing the user to book a new reservation '''
    try:
...
def release_spot(reservation_id):
    ''' We are releasing the spot by updating the leaving time, total cost and also mark the spot as available and make the reservation inactive !'''
    try:
        user_id = session['user_id']
        ...
        db.session.commit()
        flash(f"Spot {spot.spot_number} at {spot.parking_lot.location_name} released! Total: ₹{reservation.parking_cost:.2f}", 'success')
        return redirect(url_for('user_dashboard'))
    except Exception :
        db.session.rollback()
        flash(f"Error while releasing spot, Please try again", "danger")
        return redirect(url_for('user_dashboard'))
...
def user_summary():
    ...
    current_parking = Reservation.query.filter_by( user_id=session['user_id'] , leaving_time=None).first()
    prev_sessions = Reservation.query.filter( Reservation.user_id == session['user_id'] ,
    ...
    if total_sessions :
        ...
        avg_duration = f"{int(avg_seconds // 3600)}h {int((avg_seconds % 3600) // 60)}m"
    else:
    ...
    for s in prev_sessions:
        if s.parking_spot and s.parking_spot.parking_lot:
    ...
    summary = {'total_reservations': total_sessions,'total_cost': total_spent,'avg_cost': avg_cost,'avg_duration': avg_duration }
    return render_template( 'user_summary.html', summary=summary, active_reservation=current_parking, past_reservations=prev_sessions,calculate_duration=calculate_duration,
